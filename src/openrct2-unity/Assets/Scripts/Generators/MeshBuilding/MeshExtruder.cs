...
namespace OpenRCT2.Generators.MeshBuilding
{
    ...
    public class MeshExtruder
    {
        ...
        public MeshExtruder(Mesh mesh)
        {
            ...
            _uvs = mesh.uv;
            _bounds = mesh.bounds;
            _triangles = GetAllTriangles(mesh);
        }
        ...
        /// </summary>
        public float AddSegment(Vector3 positionStart, Quaternion directionStart, Vector3 positionEnd, Quaternion directionEnd, float offset, float multiplier)
        {
            ...
            int submeshCount = _triangles.Length;

            for (int s = 0; s < submeshCount; s++)
            {
                int[] submesh = _triangles[s];

                for (int t = 0; t < submesh.Length; t += 3)
                {
                    // Get index of vertices/normals for this triangle.
                    int idx1 = submesh[t];
                    int idx2 = submesh[t + 1];
                    int idx3 = submesh[t + 2];
                    ...
                    for (; step < stepCount; step++)
                    {
                        ...
                        _builder.AddTriangle(vertex1, vertex2, vertex3, s);
                    }
                }
            }
            return length;
        ...
        /// Adds an extruded segment to the final mesh. Returns the length of the segment.
        /// </summary>
        public float AddSegment(TransformPoint start, TransformPoint end, float offset, float multiplier)
            => AddSegment(start.Position, start.Rotation, end.Position, end.Rotation, offset, multiplier);
        ...
        /// <summary>
        /// Get the triangle array for each submesh in the mesh.
        /// </summary>
        static int[][] GetAllTriangles(Mesh mesh)
        {
            var count = mesh.subMeshCount;
            var submeshes = new int[count][];

            for (var idx = 0; idx < count; idx++)
            {
                submeshes[idx] = mesh.GetTriangles(idx);
            }
