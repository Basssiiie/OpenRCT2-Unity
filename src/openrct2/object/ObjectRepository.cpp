...
namespace OpenRCT2
{
    ...
    private:
        ...
        static void* CalculateExtraBytesToFixChecksum(int32_t currentChecksum, int32_t targetChecksum, size_t* outSize)
        {
            ...
            // Each set bit encountered during encoding flips one bit of the resulting checksum (so each bit of the checksum is
            // an XOR of bits from the file). Here, we take each bit that should be flipped in the checksum and set one of the
            // bits in the data that maps to it. 11 bytes is the minimum needed to touch every bit of the checksum - with less
            // than that, you wouldn't always be able to make the checksum come out to the desired target
