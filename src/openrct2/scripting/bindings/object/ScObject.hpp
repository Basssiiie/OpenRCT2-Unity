...
namespace OpenRCT2::Scripting
{
    ...
    private:
        ...
        static JSValue rotationFrameMask_get(JSContext* ctx, JSValue thisVal)
        {
            ...
            return JS_NewUint32(ctx, entry != nullptr ? entry->TabRotationMask : 0);
        }

        static JSValue spacing_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->spacing : 0);
        }

        static JSValue carMass_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->car_mass : 0);
        }

        static JSValue tabHeight_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewInt32(ctx, entry != nullptr ? entry->tab_height : 0);
        }

        static JSValue numSeats_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->num_seats : 0);
        }
        ...
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->spriteWidth : 0);
        }

        static JSValue spriteHeightNegative_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->spriteHeightNegative : 0);
        }

        static JSValue spriteHeightPositive_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->spriteHeightPositive : 0);
        }

        static JSValue animation_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? EnumValue(entry->animation) : 0);
        }
        ...
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->base_num_frames : 0);
        }

        static JSValue baseImageId_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->base_image_id : 0);
        }
        ...
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->NumCarImages : 0);
        }

        static JSValue noSeatingRows_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->no_seating_rows : 0);
        }

        static JSValue spinningInertia_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->spinning_inertia : 0);
        }

        static JSValue spinningFriction_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->spinning_friction : 0);
        }

        static JSValue frictionSoundId_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewInt32(ctx, entry != nullptr ? static_cast<int32_t>(entry->friction_sound_id) : 0);
        }

        static JSValue logFlumeReverserVehicleType_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->ReversedCarIndex : 0);
        }

        static JSValue soundRange_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? EnumValue(entry->soundRange) : 0);
        }

        static JSValue doubleSoundFrequency_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->double_sound_frequency : 0);
        }

        static JSValue poweredAcceleration_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->powered_acceleration : 0);
        }

        static JSValue poweredMaxSpeed_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->powered_max_speed : 0);
        }

        static JSValue carVisual_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->PaintStyle : 0);
        }

        static JSValue effectVisual_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->effect_visual : 0);
        }

        static JSValue drawOrder_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->draw_order : 0);
        }

        static JSValue numVerticalFramesOverride_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->num_vertical_frames_override : 0);
        }
    ...
    private:
        ...
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->images_offset : 0);
        }

        static JSValue flags_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->flags : 0);
        }
        ...
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->min_cars_in_train : 0);
        }

        static JSValue maxCarsInTrain_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->max_cars_in_train : 0);
        }

        static JSValue carsPerFlatRide_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->cars_per_flat_ride : 0);
        }

        static JSValue zeroCars_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->zero_cars : 0);
        }

        static JSValue tabVehicle_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->TabCar : 0);
        }

        static JSValue defaultVehicle_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->DefaultCar : 0);
        }

        static JSValue frontVehicle_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->FrontCar : 0);
        }

        static JSValue secondVehicle_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->SecondCar : 0);
        }

        static JSValue rearVehicle_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->RearCar : 0);
        }

        static JSValue thirdVehicle_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->ThirdCar : 0);
        }
        ...
        {
            auto entry = GetEntry(thisVal);
            return JS_NewInt32(ctx, entry != nullptr ? entry->excitement_multiplier : 0);
        }

        static JSValue intensityMultiplier_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewInt32(ctx, entry != nullptr ? entry->intensity_multiplier : 0);
        }

        static JSValue nauseaMultiplier_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewInt32(ctx, entry != nullptr ? entry->nausea_multiplier : 0);
        }

        static JSValue maxHeight_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? entry->maxHeight : 0);
        }

        static JSValue shopItem_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? EnumValue(entry->shop_item[0]) : 0);
        }

        static JSValue shopItemSecondary_get(JSContext* ctx, JSValue thisVal)
        {
            auto entry = GetEntry(thisVal);
            return JS_NewUint32(ctx, entry != nullptr ? EnumValue(entry->shop_item[1]) : 0);
        }
