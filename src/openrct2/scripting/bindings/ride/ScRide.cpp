...
namespace OpenRCT2::Scripting
{
    ...
    {
        auto ride = GetRide(thisVal);
        return JS_NewUint32(ctx, ride != nullptr ? ride->inspectionInterval : 0);
    }
    ...
    JSValue ScRide::inspectionInterval_set(JSContext* ctx, JSValue thisVal, JSValue value)
    {
        ...
        if (ride != nullptr)
        {
            ride->inspectionInterval = std::clamp<uint8_t>(
                static_cast<uint8_t>(valueInt), RIDE_INSPECTION_EVERY_10_MINUTES, RIDE_INSPECTION_NEVER);
        }
        return JS_UNDEFINED;
    }
